...
return new class extends Migration
{
    ...
    {
        // Add the "started" connection type
        // This allows events to mark the beginning of other spans
        // Forward: [event] started [span] - e.g., "Birth of JFK started JFK"
        // Inverse: [span] started by [event] - e.g., "JFK started by Birth of JFK"
        DB::table('connection_types')->insert([
            'type' => 'started',
            'forward_predicate' => 'started',
            'forward_description' => 'Started',
            'inverse_predicate' => 'started by',
            'inverse_description' => 'Started by',
    ...
    public function down(): void
    {
        ...
        if (isset($metadata['schema']['connection_type']['options'])) {
            $options = $metadata['schema']['connection_type']['options'];
            $options = array_values(array_diff($options, ['started']));
            $metadata['schema']['connection_type']['options'] = $options;
        }
