...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Recreate the function with timeless span support
        DB::statement(<<<'SQL'
            ...
            BEGIN
                ...
                SELECT * INTO span_type FROM span_types WHERE type_id = connection_span.type_id;
                is_timeless := span_type.metadata->>'timeless' = 'true';

                -- Also check if this individual span is marked as timeless
                is_timeless := is_timeless OR (connection_span.metadata->>'timeless' = 'true');
                ...
                -- If not a placeholder and not timeless, validate date precision hierarchy and ranges
                IF NOT is_placeholder AND NOT is_timeless THEN
                    -- Start date validation
                    IF connection_span.start_year IS NULL THEN
                        RAISE EXCEPTION 'Start year is required for non-placeholder, non-timeless spans';
                    END IF;
                    ...
                    END IF;
                ELSE
                    -- For placeholders or timeless spans, we still validate precision hierarchy
                    ...
                    -- For placeholders/timeless: allow end date without start date,
                ...
                IF constraint_type = 'single' THEN
                    ...
                    END IF;
                ELSIF constraint_type = 'non_overlapping' AND NOT is_placeholder AND NOT is_timeless THEN
                    -- Only check overlaps for non-placeholder, non-timeless spans
