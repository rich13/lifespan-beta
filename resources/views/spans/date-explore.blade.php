...
function renderWikipediaContent(data) {
    ...
    if (data.events && data.events.length > 0 || data.births && data.births.length > 0 || data.deaths && data.deaths.length > 0) {
        ...
        html += `Data from <a href="https://en.wikipedia.org/wiki/Wikipedia:On_this_day/{{ \Carbon\Carbon::createFromDate($year, $month, $day)->format('F_j') }}" target="_blank" class="text-decoration-none">Wikipedia On This Day</a>, not Lifespan.`;
        html += '</small>';
        html += '</div>';
    } else {
