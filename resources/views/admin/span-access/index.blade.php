...
@section('title', 'Span Access Management')
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-3">
                                    <button type="button" class="btn btn-outline-primary w-100" data-bs-toggle="modal" data-bs-target="#bulkPublicModal">
                                        <i class="bi bi-globe"></i> Make Selected Public
                                    </button>
                                </div>
                                <div class="col-md-3">
                                    <button type="button" class="btn btn-outline-secondary w-100" data-bs-toggle="modal" data-bs-target="#bulkPrivateModal">
                                        <i class="bi bi-lock"></i> Make Selected Private
                                    </button>
                                </div>
                                <div class="col-md-3">
                                    <button type="button" class="btn btn-outline-warning w-100" data-bs-toggle="modal" data-bs-target="#bulkGroupModal">
                                        <i class="bi bi-people"></i> Share with Groups
                                    </button>
                                </div>
                                <div class="col-md-3">
                                    <a href="{{ route('admin.groups.index') }}" class="btn btn-outline-info w-100">
                                            ...
                                            <tbody id="allSpansContainer">
                                                @foreach($allSpans as $span)
                                                    <tr class="span-row {{ $span->access_level }}">
                                                        ...
                                                        <td>
                                                            <span class="badge bg-{{ $span->access_level === 'public' ? 'success' : ($span->access_level === 'shared' ? 'warning' : 'danger') }}">
                                                        ...
                                                        <td>
                                                            @if($span->access_level === 'shared')
                                                                @php
                                                                    ...
                                                                    $groupPermissionsCount = $groupPermissions->count();
                                                                @endphp
                                                                @if($userPermissions > 0)
                                                                    <span class="badge bg-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $userPermissions }} user{{ $userPermissions !== 1 ? 's' : '' }} have access">{{ $userPermissions }} user{{ $userPermissions !== 1 ? 's' : '' }}</span>
                                                                @endif
                                                                @if($groupPermissionsCount > 0)
                                                                    @php
                                                                        $groupNames = $groupPermissions->pluck('group.name')->join(', ');
                                                                    @endphp
                                                                    <span class="badge bg-info" data-bs-toggle="tooltip" data-bs-placement="top" title="Groups: {{ $groupNames }}">{{ $groupPermissionsCount }} group{{ $groupPermissionsCount !== 1 ? 's' : '' }}</span>
                                                                @endif
                                                            @else
